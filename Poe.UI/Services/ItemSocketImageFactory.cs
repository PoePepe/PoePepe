...
public static class ItemSocketImageFactory
{
    ...
    private static Image CreateLink(int socketOrdinalNumber, bool isVertical = false)
    {
        ...
        var link = new Image
        {
            Source = new CroppedBitmap(GetItemSocketBitMapAsync(), linkRect),
    ...
    private static Image CreateSocket(SocketColor color)
    {
        ...
        {
            Source = new CroppedBitmap(GetItemSocketBitMapAsync(), rect),
            Margin = new Thickness(6),
            Width = ItemSocketImagePosition.SocketSize.Width,
        };
    ...
    private static PixelRect GetLinkRect(int socketOrdinalNumber, bool isVertical = false)
    {
        if (isVertical)
        {
    ...
    private static Bitmap GetItemSocketBitMapAsync()
    {
        return new Bitmap(AssetLoader.Open(ItemSocketImagePosition.SocketUri));
    }
}